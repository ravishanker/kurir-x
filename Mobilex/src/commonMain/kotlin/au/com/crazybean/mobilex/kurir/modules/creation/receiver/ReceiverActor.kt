...
class ReceiverActor(scene: ReceiverScene?,
                    wrapper: ReceiverWrapper) : Actor<ReceiverScene, ReceiverWrapper>(scene, wrapper) {
    ...
    fun onPlaceClick(name: String?, mobile: String?, email: String?) {
        when {
            ...
            mobile.isNullOrBlank() && email.isNullOrBlank() -> scene?.showNoContact()
                ...
                scene?.showSpinner()
                wrapper.createTask(origin!!, it, parcel, receiver).observe(this) {
                        scene?.hideSpinner()
                        scene?.dismiss()
                    }
            }
